...
void easeInOutExpo(ref float step, const(float) k) @safe nothrow pure
{
    ...
    step = k == 0 ? 0 : k == 1 ? 1 : k < 0.5f ? pow(2, 20 * k - 10) / 2 : (2 - pow(2,
            -20 * k + 10)) / 2;
}
...
void easeInOutCirc(ref float step, const(float) k) @safe nothrow pure
{
    ...
    step = k < 0.5f ? (1 - sqrt(1 - pow(2 * k, 2))) / 2 : (sqrt(1 - pow(-2 * k + 2, 2)) + 1) / 2;
}
...
void easeInOutBack(ref float step, const(float) k) @safe nothrow pure
{
    ...
    step = k < 0.5f ? (pow(2 * k, 2) * ((c2 + 1) * 2 * k - c2)) / 2 : (pow(2 * k - 2,
            2) * ((c2 + 1) * (k * 2 - 2) + c2) + 2) / 2;
}

void easeOutBounce(ref float step, const(float) k) @safe nothrow pure
{
    const n1 = 7.5625;
    const d1 = 2.75;
    ...
    if (x < 1 / d1) {
        ...
        step = n1 * (x -= 1.5 / d1) * x + 0.75;
    } else if (x < 2.5 / d1) {
        step = n1 * (x -= 2.25 / d1) * x + 0.9375;
    } else {
        step = n1 * (x -= 2.625 / d1) * x + 0.984375;
    }
}
